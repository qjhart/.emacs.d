...
See which key is used to complete a command
#+begin_src emacs-lisp
...
org-mode can break if the tab-width is changed. See [[https://github.com/doomemacs/doomemacs/issues/7670][this issue]].
...
[[https://orgmode.org/manual/Agenda-Views.html][Agendas]] are a standard part of
=org-mode= and can be used to find =TODOS= etc. You can extend the standard
...
** Debugging
 ...
 [[https://github.com/joaotavora/eglot][=elgot=]], as well as the debugger =dape=
...
*** =dape=
    ...
    [[https://github.com/svaante/dape][dape]] is a debugger. I followed their install instructions for javascript,
...
*** =elgot=
    ...
    #+RESULTS:
    : ((bash-mode . bash-ts-mode) (js-mode . js-ts-mode))
...
I think that sparql-mode just assumes that ob-http (which is super old), just
exists.
#+begin_src emacs-lisp
  (use-package ob-http)
#+end_src
...
feature, you can also use eshell within a tramp docker session.   This is
standard in emacs now
...
#+RESULTS:
| use-package | transient/:catch: Symbol’s value as variable is void: _ | :error | nil |
